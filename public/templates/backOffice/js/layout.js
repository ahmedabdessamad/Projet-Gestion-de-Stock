...
$(document).ready(function() {
    ...
    $(function() {
        ...
        url = path;
        level = path[path.length-2];
        path = path[path.length-1];
        if (path !== undefined) {
            element = $("#sidebar-menu").find("a[href$='" + path + "']").first();
            element.addClass('sfActive');
            element.parents().eq(3).superclick('show');
            if(element.length!=0) return true;
        }
        if(level !== undefined && $("#sidebar-menu").find("a[href$='" + path + "']")){
            element = $("#sidebar-menu").find("a[href$='" + level + "']").first();
            element.addClass('sfActive');
            element.parents().eq(3).superclick('show');
            if(element.length!=0) return true;
            path = url[url.length-3];
            if(path !== undefined && $("#sidebar-menu").find("a[href$='" + level + "']")){
                element = $("#sidebar-menu").find("a[href$='" + path + "']").first();
                element.addClass('sfActive');
                element.parents().eq(3).superclick('show');
            }
